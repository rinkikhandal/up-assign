...
export const Navbar = () => {
  ...
  const handleNavbarClick = (e, href, close) => {
    if (href === "/") {
      close(); // Close DisclosurePanel when the link is clicked
      return;
    }
    if (!user && !localUser) {
      e.preventDefault();
  ...
  return (
    <>
      ...
      <Disclosure
        as="nav"
        className="z-50 fixed left-0 right-0 top-0 bg-white drop-shadow-xl"
      >
        {({ open, close }) => (
          <>
            <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
              <div className="relative flex h-16 items-center justify-between">
                <div className="absolute inset-y-0 left-0 flex items-center lg:hidden">
                  ...
                  <DisclosureButton
                    ref={menuClose}
                    className="group relative inline-flex items-center justify-center rounded-md p-2 text-gray-500 hover:bg-gray-700 hover:text-white focus:ring-2 focus:ring-white focus:outline-none focus:ring-inset"
                  >
                    ...
                    <div className="flex space-x-4 text-xs">
                      {navItems.map((item) => (
                        <Link
                          ...
                          to={item.href}
                          onClick={(e) =>
                            handleNavbarClick(e, item.href, close)
                          } // Pass `close`
              ...
              <div className="space-y-1 px-2 pt-2 pb-3 text-xs">
                {navItems.map((item) => (
                  <Link
                    key={item.name}
                    to={item.href}
                    onClick={(e) => handleNavbarClick(e, item.href, close)} // Pass `close`
                    title={
                      item.name !== "Dashboard"
